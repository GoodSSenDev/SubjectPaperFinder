...
const queuedPaperSchema = new Schema({
    _PId: { type: Number, required: true, unique: true, },
...
exports.insertNewQueuedPaper = async (paper) => {
    ...
    if (QueuedPapersNumber == 0) {
        await QueuedPapers.collection.insertOne({ _PId: 0, ...paper }).catch((err) => {
            console.log(err);
            return false;
        });
    }
    else {
        const { _PId } = await QueuedPapers.find().sort({ _id: -1 }).limit(1);
        await QueuedPapers.collection.insertOne({ _PId: (_PId + 1), ...paper }).catch((err) => {
            console.log(err);
            return false;
        });
    }
